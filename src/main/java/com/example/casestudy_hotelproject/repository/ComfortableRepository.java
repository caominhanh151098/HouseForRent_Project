...
public interface ComfortableRepository extends JpaRepository<Comfortable, Integer> {
    ...
    @Query(value = "SELECT c.* FROM comfortable c JOIN comfortable_detail cd ON c.id = cd.comfortable_id WHERE cd.house_id = :houseId AND cd.status = TRUE LIMIT 10",nativeQuery = true)
